...
$events_previous_month_text = 'prev'; // e.g. 'prev &lt;&lt;'
$events_next_month_text = 'next'; // e.g. '&lt;&lt; next'
...
function events_preload()
{
	...
	{
		$events_base_url = htmlentities($_SERVER['PHP_SELF'], ENT_QUOTES).'?id=events';
	}
...
function events_form()
{
	...
	// latest event id
	$latest_event_id = 0;
	foreach ($events_xml->xpath('//events/event/@event_id') as $el)
	{
	  $i = intval($el->event_id);
	  if ($i > $latest_event_id) $latest_event_id = $i;
	}
	// new event id
	...
	$events_calendar_date_display = gmstrftime('%d %B %Y', $events_calendar_date);
	// store as number
...
FORM;
	...
	if($new_event)
	{
		...
		{
			echo '<p>Go to <a href="'.$events_base_url.'today">Today</a> ('.gmstrftime('%d %B %Y', $today).')</p>';
		}
	...
	else
	{
		...
		if(gmdate('j n Y', $today) != gmdate('j n Y', $events_calendar_date))
		{
			echo '<p>Go to <a href="'.$events_base_url.'">Today</a> ('.gmstrftime('%d %B %Y', $today).')</p>';
		}
		echo $calendar->render('event_day_render', 'event_caption_render');
	}
}
...
function event_caption_render($event_date)
{
	...
	// set to midnight
	$previous_month = gmmktime(0, 0, 0, gmdate('n', $previous_month), gmdate('j', $previous_month), gmdate('Y', $previous_month));
	
	if(strlen($events_previous_month_text) == 0)
	{
		$events_previous_month_text = gmstrftime('%B %Y', $previous_month);
	...
	// set to midnight
	$next_month = gmmktime(0, 0, 0, gmdate('n', $next_month), gmdate('j', $next_month), gmdate('Y', $next_month));
	
	if(strlen($events_next_month_text) == 0)
	{
		$events_next_month_text = gmstrftime('%B %Y', $next_month);
	...
	if($event_count > 0)
	{
		...
		foreach($events as $event)
		{
			...
			if(!in_array($date_formatted, $date_headers))
			{
				...
				{
					$list .= '<'.$date_heading_tag .'><a href="'.$base_url.'month='.gmdate('n', $event_date).'&year='.gmdate('Y', $event_date).'">'.gmstrftime('%B %Y', $event_date).'</a> &raquo; '.$date_formatted.'</'.$date_heading_tag.'>';
				}
				if(!empty($_GET['day']))
				{
...
EVENT;
	...
	{
		$output = 'No events '.(isset($_GET['day']) ? ' on '.gmstrftime('%d %B %Y', $events_calendar_date) : ' in '.gmstrftime('%B %Y', $events_calendar_date));
	}
